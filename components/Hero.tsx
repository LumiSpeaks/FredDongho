...
const Hero: React.FC = () => {
  // #region agent log
  const headshotPath = RESUME_DATA.headshot;
  fetch('http://127.0.0.1:7243/ingest/3dca8239-0aa4-4d98-974c-66e531909bf9',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'Hero.tsx:6',message:'Hero component rendered',data:{headshotPath},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
  // #endregion

  const handleImageLoad = () => {
    // #region agent log
    fetch('http://127.0.0.1:7243/ingest/3dca8239-0aa4-4d98-974c-66e531909bf9',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'Hero.tsx:handleImageLoad',message:'Image loaded successfully',data:{headshotPath},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
    // #endregion
  };
  ...
  const handleImageError = (e: React.SyntheticEvent<HTMLImageElement, Event>) => {
    ...
    fetch('http://127.0.0.1:7243/ingest/3dca8239-0aa4-4d98-974c-66e531909bf9',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'Hero.tsx:handleImageError',message:'Image failed to load',data:{headshotPath,src:target.src,error:'404 or file not found'},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
    // #endregion
  };
      ...
      <div className="max-w-6xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
        ...
        <div className="text-left animate-in fade-in slide-in-from-left-8 duration-1000 order-2 lg:order-1">
          ...
          <div className="flex flex-col sm:flex-row items-start lg:items-center gap-6">
            <a href="#acestudy" className="px-8 py-4 bg-[#1d1d1f] text-white rounded-full font-bold hover:bg-[#007aff] hover:scale-105 transition-all shadow-[0_10px_30px_rgba(29,29,31,0.3)] text-base relative overflow-hidden group">
            ...
            <a href="#journey" className="px-6 py-3 border-2 border-[#1d1d1f] text-[#1d1d1f] hover:bg-[#1d1d1f] hover:text-white transition-all font-bold text-base flex items-center gap-2 group rounded-full">
              My Journey 
        ...
        <div className="relative order-1 lg:order-2 flex justify-center lg:justify-end animate-in fade-in slide-in-from-right-8 duration-1000 delay-200">
          <div className="relative group">
            ...
            {/* Black Accent Decorative Elements - Behind Image */}
            ...
            {/* The Image Container - Black Frame */}
