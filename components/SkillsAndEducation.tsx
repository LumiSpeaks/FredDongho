...
const SkillsAndEducation: React.FC = () => {
  const { education, skillGroups } = RESUME_DATA;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          <div className="lg:col-span-7 glass p-4 md:p-6 rounded-[1.25rem] relative overflow-hidden group bg-white border border-neutral-200">
            ...
            <div className="mb-5">
              ...
              <p className="text-xs text-neutral-500 mb-1">{education.location}</p>
              <p className="text-sm text-neutral-600 font-medium mb-2">{education.degree}</p>
              <p className="text-xs font-semibold text-neutral-700 mb-2">
                Emphasis: {education.emphasis}
              </p>
              <div className="flex flex-wrap gap-2 items-center">
                <span className="px-2 py-0.5 rounded-full bg-[#007aff] text-white text-[8px] font-black uppercase tracking-widest">
            ...
            <div className="space-y-3">
              <h4 className="text-[8px] font-black uppercase tracking-[0.3em] text-neutral-400">
                Honors & Distinctions
              </h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {education.honors.map((honor, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-2 bg-neutral-50 p-2 rounded-xl border border-neutral-200 hover:bg-[#1d1d1f] hover:border-[#1d1d1f] transition-all group/honor shadow-sm hover:shadow-[0_4px_12px_rgba(29,29,31,0.2)]"
                  >
                    ...
                    <span className="text-xs font-bold text-neutral-600 group-hover/honor:text-white transition-colors">
                      {honor}
                    </span>
                  </div>
                ))}
          ...
          <div className="lg:col-span-5 glass p-4 md:p-6 rounded-[1.25rem] flex flex-col justify-between bg-white border border-neutral-200">
            <div>
              <h2 className="text-base font-bold tracking-tight mb-5 text-[#1d1d1f]">Technical skills</h2>
              <div className="space-y-4">
                {skillGroups.map((group) => (
                  <div key={group.name}>
                    <h3 className="text-[10px] font-black uppercase tracking-widest text-neutral-400 mb-2">
                      {group.name}
                    </h3>
                    <div className="flex flex-wrap gap-1.5">
                      {group.items.map((item) => (
                        <span
                          key={item}
                          className="px-2 py-1 rounded-xl bg-neutral-50 text-[#1d1d1f] border border-neutral-200 font-bold text-[10px] md:text-xs hover:bg-[#1d1d1f] hover:text-white transition-all"
                        >
                          {item}
                        </span>
                      ))}
            ...
            <div className="mt-6 p-3 rounded-xl bg-gradient-to-br from-[#1d1d1f] to-[#000000] text-white shadow-[0_5px_20px_rgba(29,29,31,0.3)] relative overflow-hidden group/quote">
              ...
              <p className="text-xs font-black italic leading-snug relative z-10">
                &ldquo;Focused on building products that solve real problems.&rdquo;
              </p>
            </div>
          </div>
        </div>
